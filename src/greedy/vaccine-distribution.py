...
def solve(n, d, ages):
    ...
    ages.sort()
    risk_count = sum(1 for i in ages if i <= 9 or i >= 80)
    safe_count = sum(1 for i in ages if 9 < i < 80)

    total = safe_count // d + (1 if safe_count % d != 0 else 0)
    total += risk_count // d + (1 if risk_count % d != 0 else 0)
