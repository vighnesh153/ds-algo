...
def my_index(arr, val, from_left):
    if from_left:
...
def solve(n, x, p, k, nums):
    ...
    if x_exists:
        if nums[p] == x:
            return 0
        x_index = my_index(nums, x, True)
        if x_index < p:
            return -1 if k < p else (p - my_index(nums, x, False))
        elif x_index > p:
            return -1 if k > p else (x_index - p)
    else:
        nums[k] = x
        ans = solve(n, x, p, k, nums)
        return -1 if ans == -1 else (ans + 1)


def main(*args):
    for _ in range(integer_input()):
        n, x, p, k = integer_array_input()
        nums = integer_array_input()
        res = solve(n, x, p - 1, k - 1, nums)
        print(res)
