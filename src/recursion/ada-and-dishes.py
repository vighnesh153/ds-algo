...
def recursion(arr, index, result, s1=0, s2=0):
    if index == arr.length:
        diff = abs(s1 - s2)
        if diff <= result[0]:
            result[1] = min(max(s1, s2), result[1])
            result[0] = diff
        return

    recursion(arr, index + 1, result, s1 + arr[index], s2)
    recursion(arr, index + 1, result, s1, s2 + arr[index])


def solve(times):
    s = sum(times)
    result = [s, s]
    recursion(times, 0, result)
    print(result[1])
