...
def just_minimum_array(arr):
    result = MyList.of([arr.length], -1)
    stack = MyList()
    i = arr.length - 1
    while i >= 0:
        while not stack.is_empty and arr[i] < arr[stack.last]:
            result[stack.pop()] = i
        stack.push(i)
        i -= 1
...
def max_histogram_area(arr):
    left_just_min = just_minimum_array(arr)
    right_just_min = just_minimum_array(MyList(arr[::-1]))
    right_just_min = [arr.length
                      if i == -1
                      else arr.length - i - 1
                      for i in right_just_min[::-1]]

    this = This()
    for i in range(arr.length):
        left = left_just_min[i] + 1
        right = right_just_min[i] - 1
        this.update_if_max((right - left + 1) * arr[i])
...
def solve():
    while True:
        ...
        if arr.length == 1 and arr[0] == 0:
            break
        print(max_histogram_area(MyList(arr[1:])))
