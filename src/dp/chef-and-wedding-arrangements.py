...
def get_quarrel_cost(selected):
    return sum([0] + [c for c in Counter(selected).values() if c > 1])


def recursion(index, selected, cost, families, k):
    if index == families.length:
        return cost + get_quarrel_cost(selected) + k

    selected.append(families[index])
    cost_1 = recursion(index + 1, selected, cost, families, k)
    selected.pop()
    cost_2 = recursion(index + 1, [families[index]], cost, families, k)
    cost_2 += get_quarrel_cost(selected) + k

    return min(cost_1, cost_2)


def solve(k, fam):
    print(recursion(0, [], 0, fam, k))
...
def main(*args):
    ...
    for _ in test_case_count():
        _, k = integer_array_input()
        fam = integer_array_input()
        solve(k, fam)
